...
namespace AudioPPM
{
    public class SoundPlay : IDisposable
    {
        ...
        public byte[] GenPPM()
        {
            frame.Clear(); // removes all elements from frame 
            //PPM "Stop/Start"
            try
            {
                frame.AddRange(Enumerable.Repeat((short)Amplitude, (int)(102 * microsec * channels)));
            ...
            lock (channels_lock)
            {
                foreach (int i in PPMchannels.Keys)
                {
                    try
                    {
                        ...
                        // Enumerable.repeat = repeats the value returns a collection with repeated elements. 
                        // In the first argument,we specify a value—this is repeated the number of times specified in the second argument.
                        
                        //PPM base
                        frame.AddRange(Enumerable.Repeat(Amplitude, (int)(7 * microsec * channels)));
                        //PPM Signal
                        frame.AddRange(Enumerable.Repeat(Amplitude, (int)((PPMchannels[i] * 0.8 / 100) * 10 * microsec * channels)));
                        //PPM Signal end
                        frame.AddRange(Enumerable.Repeat((short)-Amplitude, (int)(4 * microsec * channels)));
                    }
                    catch { }
